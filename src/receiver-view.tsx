...
export function ReceiverView() {
  ...
  const handleManualLoad = () => {
    try {
      ...
      // Check for the new array structure
      if (parsedLogic.name && parsedLogic.triggers && parsedLogic.actions) {
        handleLogicScanned(parsedLogic);
        setManualJson('');
      } else {
        throw new Error('Invalid logic structure. Must include name, triggers, and actions.');
      }
    } catch (e) {
      toast({
        title: "Invalid JSON",
